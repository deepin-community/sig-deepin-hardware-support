...
#define __RTW89_ASSERT_CHIP_INFO(x, t) \
void rtw89_assert_chip_info_ ## t(void); \
void rtw89_assert_chip_info_ ## t(void) \
{ \
	BUILD_BUG_ON(x.bacam_num > RTW89_MAX_BA_CAM_NUM); \
	BUILD_BUG_ON(x.support_chanctx_num > NUM_OF_RTW89_SUB_ENTITY); \
	BUILD_BUG_ON(x.dflt_parms != &rtw89_ ## t ## _dflt_parms); \
	BUILD_BUG_ON(!x.ops->set_channel); \
	BUILD_BUG_ON(!x.ops->set_txpwr); \
}

#define RTW89_ASSERT_CHIP_INFO(t) \
	__RTW89_ASSERT_CHIP_INFO(rtw ## t ## _chip_info, t)
...
_ieee80211_set_sband_iftype_data(struct ieee80211_supported_band *sband,
				 ...
				 u16 n_iftd)
{
	sband->iftype_data = (const void __force *)iftd;
...
#define ieee80211_nullfunc_get(__hw, __vif, __link_id, __qos_ok) \
	ieee80211_nullfunc_get(__hw, __vif, __qos_ok)
...
#define BP_STA(sta) ((struct __bp_sta *)sta)

struct __bp_sta_agg {
	struct __deflink {
		struct ieee80211_sta agg;
	} deflink;
};

#define BP_STA_AGG(sta) ((struct __bp_sta_agg *)sta)




#ifndef IEEE80211_HE_MAC_CAP3_MAX_AMPDU_LEN_EXP_EXT_2
...
#endif
#ifndef IEEE80211_HE_PHY_CAP9_NOMINAL_PKT_PADDING_MASK
...
#define RTW89_RADIOTAP_ROOM_EHT sizeof(void *)
...
static inline
void rtw89_chip_bb_postinit(struct rtw89_dev *rtwdev, enum rtw89_phy_idx phy_idx)
{
	const struct rtw89_chip_info *chip = rtwdev->chip;

	if (chip->ops->bb_postinit)
		chip->ops->bb_postinit(rtwdev, phy_idx);
}
