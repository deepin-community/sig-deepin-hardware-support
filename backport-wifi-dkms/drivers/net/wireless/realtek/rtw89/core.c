					    ...
					    struct rtw89_rx_phy_ppdu *phy_ppdu)
{
	...
	ie = le32_get_bits(iehdr->w0, RTW89_PHY_STS_IEHDR_TYPE);
	if (ie != RTW89_PHYSTS_IE01_CMN_OFDM && !phy_ppdu->to_self)
		return 0;
					...
					struct ieee80211_rx_status *rx_status)
{
	...
	if (data_rate_mode == DATA_RATE_MODE_NON_HT) {
		...
		/* encoding borrow from VHT */
		#define RX_ENC_EHT RX_ENC_VHT
		rx_status->encoding = RX_ENC_EHT;
		rx_status->rate_idx = rtw89_get_data_mcs(rtwdev, data_rate);
		rx_status->nss = rtw89_get_data_nss(rtwdev, data_rate) + 1;
		eht = true;
		/* encoding borrow from VHT */
		rx_status->rate_idx = min_t(u8, rx_status->rate_idx, 9);
	} else {
		rtw89_warn(rtwdev, "invalid RX rate mode %d\n", data_rate_mode);
	}
...
int rtw89_core_start(struct rtw89_dev *rtwdev)
{
	...
	rtw89_chip_bb_postinit(rtwdev, RTW89_PHY_0);
